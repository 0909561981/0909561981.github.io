class Map {
    ...
    // 建立遊戲障礙物   ( wall  spike  well)
    build() {
        // 建立遊戲邊界障礙物   -- wall
        const spacing = 10;
        for (let x = 0; x < this.width; x += spacing) {
            this.tiles.push(new Tile(x, 0, TileType.WALL));
            this.tiles.push(new Tile(x, this.height - 1, TileType.WALL));
        }
        for (let y = spacing; y < this.height - spacing; y += spacing) {
            this.tiles.push(new Tile(0, y, TileType.WALL));
            this.tiles.push(new Tile(this.width - 1, y, TileType.WALL));
        }

        // 建立有刺障礙物       -- spike
        this.tiles.push(new Tile(this.width / 2 - this.width * 3 / 16, this.height / 2, TileType.SPIKE));
        this.tiles.push(new Tile(this.width / 2 + this.width * 3 / 16, this.height / 2, TileType.SPIKE));
    ...
    is_walkable(pos) {
        ...
        return { walkable: true, type: TileType.GROUND, pos: { x: 878787, y: 878787 } };
    }
}