...
async function showUpgradeScreen() {
  ...
  // 使用後端回傳的升級點數
  let upgradePoints = 7-(playerStats.max_health+playerStats.movement_speed+playerStats.bullet_damage+playerStats.body_damage+playerStats.bullet_frequency+playerStats.health_regen+playerStats.bullet_speed)+ (int)(playerStats.ranking_lv);
  if(upgradePoints<0)
    upgradePoints=0;
  // 初始化等級從後端數值四捨五入
  const levels = {
